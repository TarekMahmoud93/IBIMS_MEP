...
namespace IBIMS_MEP
{
    ...
    public class Hangers : IExternalCommand
    {
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet element)
        {
            ...
            IList<Element> MechanicalEquipment = fec6.WherePasses(ecf6).ToList(); FamilySymbol rfs1 = null; FamilySymbol pfs = null; FamilySymbol rfs2 = null;
            FamilySymbol fsd1 = null;
            foreach (Element e in MechanicalEquipment)
            {
                ...
                if (rfs1 != null && pfs != null && fsd1 != null && rfs2 != null) { break; }
            }
            string nullfams = "";
            if (rfs1 == null)
            {
            ...
            if (fsd1 == null || rfs1 == null || rfs2 == null || pfs == null)
            {
                TaskDialog.Show("Error", "Please Load Supports Family." + "\n" + nullfams); ;
                return Result.Failed;
            }
            List<string> LinkesNames = new List<string>();
            ...
            {
                RevitLinkType rlt = (RevitLinkType)doc.GetElement(ri.GetTypeId());
                LinkesNames.Add(rlt.Name);
                if (rlt.Name == LinkesNames[f7.lnk])
                {
            ...
            using (Transaction trans = new Transaction(doc, "IBIMS_Hangers"))
            {
                trans.Start();
                int h = 0;
                /*fsd2.Activate();*/ rfs1.Activate(); fsd1.Activate(); pfs.Activate(); rfs2.Activate(); 
                foreach (Element d in DUCTS)
                {
                    if (hangptsss[h].Count > 0)
                    {
                        foreach (XYZ p in hangptsss[h])
                        {
                            ...
                            FamilyInstance hang = doc.Create.NewFamilyInstance(PP, fsd1, HOs[h], d, StructuralType.NonStructural);
                ...
                foreach (Element pip in PIPES)
                {
                    ...
                    foreach (XYZ p in pangptsss[pir])
                    {
                        ...
                        if (hu < hd && fflu!=-1)
                        {
                            if (Firebool[pir])
                            {
                                FamilyInstance pangq = doc.Create.NewFamilyInstance(p, pfs, FOps[pir], doc.GetElement(plvlids[pir]), StructuralType.NonStructural); double q = 1;
                ...
                foreach (Element ct in CTS)
                {
                    foreach (XYZ p in CTPS[ctco])
                    {
                        ...
                        if (fflu != -1)
                        {
                            XYZ PP = new XYZ(p.X, p.Y, pelevsct[ctco]); fsd1.Activate();
                            FamilyInstance hang = doc.Create.NewFamilyInstance(PP, fsd1, FOcts[ctco], doc.GetElement(plvids[ctco]), StructuralType.NonStructural);
